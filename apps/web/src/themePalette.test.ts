...
describe("theme files", () => {
  ...
  it("includes Nord as a readable dark-only maintainer theme", () => {
    expect(getThemeDefinition(NORD_THEME.id)).toBe(NORD_THEME);
    expect(getThemeModes(NORD_THEME)).toEqual(["dark"]);
    expect(contrastRatio(NORD_THEME.colors.text, NORD_THEME.colors.canvas)).toBeGreaterThanOrEqual(
      4.5,
    );
    expect(
      contrastRatio(NORD_THEME.colors.accentForeground, NORD_THEME.colors.accent),
    ).toBeGreaterThanOrEqual(4.5);
  });
